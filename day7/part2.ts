...
let best = maxSize
const wantedFreeSpace = 30000000
const currentSize = fileSystem!.getSize()

console.log({currentSize})

const evaluateFolder = (size: number) => {
    const remainSpace = maxSize - currentSize + size
    if (remainSpace >= wantedFreeSpace && size < best) {
        best = size 
    }
}
